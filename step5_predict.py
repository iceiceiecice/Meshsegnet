...
if __name__ == '__main__':
    ...
    model_path = './models'
    model_name = 'MeshSegNet_Man_15_classes_72samples_lr1e-2_best.zip'

    mesh_path = './'  # need to define
    sample_filenames = ['l_201911070015.stl'] # need to define
    output_path = './outputs'
    ...
    with torch.no_grad():
        for i_sample in sample_filenames:
            ...
            for i in range(len(cells)):
                cells[i][0], cells[i][1], cells[i][2] = mesh_d._data.GetPoint(mesh_d._data.GetCell(i).GetPointId(0)) # don't need to copy
                cells[i][3], cells[i][4], cells[i][5] = mesh_d._data.GetPoint(mesh_d._data.GetCell(i).GetPointId(1)) # don't need to copy
                cells[i][6], cells[i][7], cells[i][8] = mesh_d._data.GetPoint(mesh_d._data.GetCell(i).GetPointId(2)) # don't need to copy
